...
startActivityForResult(intent, PICK_CONTACT);
		...
		cbtn.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				CheckBox cb=(CheckBox) findViewById(R.id.checkBoxc1);

				if (title.getText().toString().equals("")
						|| phonnum.getText().toString().equals("")
						|| date.getText().toString().equals("")
						|| time.getText().toString().equals("")) {
					Toast.makeText(CallReminderActivity.this, "Invalid Input",
							Toast.LENGTH_SHORT).show();
				} 	try {
					if(com.ssr.bl.DateTimeProc.isInValidDateTime( date.getText().toString()
							,time.getText().toString())){
						Toast.makeText(CallReminderActivity.this,
								"Invalid Date Time, Date Time cant be older. Reminder not created.", Toast.LENGTH_SHORT).show();
					}else {
						String showp="false";
						if(cb.isChecked())showp="true";
						
						CallReminder mr = new CallReminder();
						mr.createCallReminder(title.getText().toString(), phonnum
								.getText().toString(), date.getText().toString(),
								time.getText().toString(),
								showp,
								CallReminderActivity.this);
